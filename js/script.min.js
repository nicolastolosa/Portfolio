...
$(window).on('scroll',function(){counter();});$('#experienceModal').on('show.bs.modal',function(event){var button=$(event.relatedTarget)
var job=button.data('job')
var duration=button.data('duration')
var company=button.data('company')
var logo=button.data('logo')
var company_desc=button.data('company_desc')
var objective=button.data('objective')
var tasks=button.data('tasks')
var modal=$(this)
modal.find('.header-title').text(job)
modal.find('.header-suptitle').text(company+' | '+duration)
modal.find('.modal-logo').attr('src',logo)
modal.find('.company-description').text(company_desc)
modal.find('.job-objective').text(objective)
modal.find('.job-tasks').html(tasks);})
function smoothScroll(){$('a[href*="#"]').not('[href="#"]').not('[href="#0"]').on("click",function(event){if(location.pathname.replace(/^\//,'')==this.pathname.replace(/^\//,'')&&location.hostname==this.hostname){var target=$(this.hash);target=target.length?target:$('[name='+this.hash.slice(1)+']');if(target.length){event.preventDefault();$('html, body').animate({scrollTop:target.offset().top},1000,"easeInOutExpo",function(){var $target=$(target);$target.focus();if($target.is(":focus")){return false;}else{$target.attr('tabindex','-1');$target.focus();}});}}});jQuery.extend(jQuery.easing,{easeInOutExpo:function(x,t,b,c,d){if(t==0)return b;if(t==d)return b+c;if((t/=d/2)<1)return c/2*Math.pow(2,10*(t-1))+b;return c/2*(-Math.pow(2,-10*--t)+2)+b;}});}
if(navigator.userAgent.indexOf('Opera Mini')==-1||navigator.userAgent.indexOf('UCBrowser')==-1){smoothScroll();}
var backgroundSlide=$('#background-slide');backgroundSlide.owlCarousel({loop:true,items:1,dots:false,autoplay:true,autoplayTimeout:5000,animateOut:'fadeOut'});function SliderBackground(){if($(".owl-full-width .slider").length){$(".owl-full-width .slider").each(function(){var $this=$(this);var img=$this.children(img);var imgSrc=img.attr("src");$this.css({backgroundImage:"url("+imgSrc+")",backgroundSize:"cover",backgroundPosition:"center center"});});}}
SliderBackground();$(window).on('scroll',function(){var counter=0;$('section').each(function(){if($(window).scrollTop()+80>=$(this).offset().top){$('.menu-item').removeClass('active');$(".menu-item."+counter).addClass("active");$(".mobile-menu-item").removeClass("active");$(".mobile-menu-item."+counter).addClass("active");}
